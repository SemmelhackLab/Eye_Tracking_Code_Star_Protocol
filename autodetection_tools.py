...
#         threshold_line = []
#         for i in binocular:
#             threshold_line.append(40) #set the threshold line for eye movement
...
#             tail_threshold_line.append(40) #set the threshold line for tail_movement
...
#         threshold_line = []
#         for i in binocular:
#             threshold_line.append(45) #set the threshold line for eye movement
#         plt.plot(threshold_line,'g')
#         plt.ylim(0,max(binocular)+10)   # set the y-range for the ploting
#         plt.xlim(0,len(sb_mid)+10)   # set the x-range for the ploting
...
#
#         plt.title('Binocular convergence',fontsize=25)
#         plt.subplots_adjust(wspace=0.5, hspace=0.5)
#         if striking_candidates!= []:
#             for frame in np.array(striking_candidates)[:, 0]:
...
#         plt.xticks(fontsize=20)
#         plt.yticks(fontsize=20)
#         plt.title('swim bladder actual moving distance',fontsize=20)
#         plt.subplots_adjust(wspace=0.5, hspace=0.5)
#         if striking_candidates!= []:
#             for frame in np.array(striking_candidates)[:, 0]:
...
#         plt.yticks(fontsize=20)
#
#         plt.title('changing rate of swim bladder movement',fontsize=20)
#         if striking_candidates!= []:
#             for frame in np.array(striking_candidates)[:, 0]:
...
#         plt.xticks(fontsize=20)
#         plt.yticks(fontsize=20)
#         plt.title('tail_bend_amplitude(absolute)',fontsize=20)
#         if annotation_list != []:
#             for i in annotation_list:
#                 plt.scatter(i, tail_amplitude_list[i], marker='v', c='g', edgecolors='face', s=50)
#
...
#                   if binocular[mid] > 45:  #to make sure the binocular eyes convergence is bigger than 30
...
#                 break
#             if striking_frame[i + 1][0] - striking_frame[i][0] < 100:  # two strikes can not occur within an interval less than 30 frame during 300Hz sampling
...
#                     if i >= len(striking_frame) - 1:
#                         break
#                     if striking_frame[i + 1][0] - striking_frame[i][0] < 50:  # two strikes can not occur within an interval less than 30 frame during 300Hz sampling
#                         mid_x = (striking_frame[i + 1][0] + striking_frame[i][0]) / 2
#                         mid_y = data[mid_x]
#                         striking_frame[i] = (mid_x, mid_y)
#
...
def plot_strike_candidates(binocular, sb_mid, sb_cr, tail_amplitude_list, striking_candidates,binocular_thresh=0, mouth_area = None, video = None, mouth_ana = '', annotation_list = []):
    ...
    if mouth_ana == 'y':
        ...
        threshold_line = []
        for i in binocular:
            threshold_line.append(42) #set the threshold line for eye movement
        plt.plot(threshold_line,'g')
        plt.ylim(0,max(binocular)+10)   # set the y-range for the ploting
        plt.xlim(0,len(sb_mid)+10)   # set the x-range for the ploting
        ...
        fig=plt.figure(figsize=(8, 6))  # try to display a big figure
        plt.style.use('dark_background')
        #fig = plt.figure(figsize=(8, 6))
        ax1 = fig.add_subplot(411)  # the first subplot in the first figure
        right_side = ax1.spines["right"]
        top_side = ax1.spines["top"]
        right_side.set_visible(False)
        top_side.set_visible(False)
        plt.plot(binocular,color='#FFDDE2FF',linewidth=0.3)
        #line1, =ax1.plot([],[],color='#FFDDE2FF',lw=1)

        threshold_line = []
        for i in binocular:
            threshold_line.append(binocular_thresh) #set the threshold line for eye movement
        plt.plot(threshold_line,'#F9DC5CFF')
        plt.ylim(0,max(binocular)+10)   # set the y-range for the ploting
        plt.xlim(0,len(sb_mid)+10)   # set the x-range for the ploting
        ...
        ax2= fig.add_subplot(412)  # the second subplot in the first figure
        right_side = ax2.spines["right"]
        top_side = ax2.spines["top"]
        right_side.set_visible(False)
        top_side.set_visible(False)
        sb_mid= gaussian_filter(sb_mid, sigma=3, mode='nearest')
        sb_cr= gaussian_filter(sb_cr, sigma=3, mode='nearest')
        plt.plot(sb_mid,color='#FAA094FF',linewidth=0.5)
        #line2, = ax2.plot([], [], color='#FAA094FF', lw=1)
        plt.xlim(0,len(sb_mid)+10)   # set the x-range for the ploting
        plt.title('swim bladder actual moving distance')
        plt.subplots_adjust(wspace=0.5, hspace=0.5)
        if striking_candidates!= []:
        ...
        ax3 = fig.add_subplot(413)  # the third subplot in the first figure
        right_side = ax3.spines["right"]
        top_side = ax3.spines["top"]
        right_side.set_visible(False)
        top_side.set_visible(False)
        plt.plot(sb_cr,color='#9ED9CCFF',linewidth=0.5)
        #line3, = ax3.plot([], [], color='#9ED9CCFF', lw=2)
        plt.xlim(0,len(sb_mid)+10)   # set the x-range for the ploting
        plt.title('changing rate of swim bladder movement')
        if striking_candidates!= []:
            for frame in np.array(striking_candidates)[:, 0]:
        ...
        ax4 = fig.add_subplot(414)  # the third subplot in the first figure
        right_side = ax4.spines["right"]
        top_side = ax4.spines["top"]
        right_side.set_visible(False)
        top_side.set_visible(False)
        plt.plot(tail_amplitude_list,color='#008C76FF',linewidth=1)
        #line3, = ax4.plot([], [], color='#008C76FF', lw=2)
        plt.plot(50,'g') #50 degree here is the amplitude threshold
        plt.xlim(0, len(tail_amplitude_list) + 10)  # set the x-range for the ploting
...
def striking_detection(data, binocular, peak_thresh=0.03, temporal_thresh=15, amplitude_thresh=0.2,binocular_thresh=0):
    # input data has to be an iterate, either list or tuple
    # peak_thresh is the threshold for detecting peaks in function peakdetector.peakdetold(data,0.15)
    # temporal_thresh specifies the positive peak has to be within 'temporal_thresh' frames right after negative peaks
    # amplitude_thresh specifies the value for positive peak has to be 'amplitude_thresh' bigger than the negative peaks
    ...
    for i, p in enumerate(n_p):
        for sub_i, sub_p in enumerate(p_p):
            if 0 < p_p[sub_i][0] - n_p[i][0] < temporal_thresh:  # temporal threshold! #the positive peak has to be within 'temporal_thresh' frames right after negative peaks
               if p_p[sub_i][1] - n_p[i][1] > amplitude_thresh:  # peak amplitude threshold! #the value for positive peak has to be 'amplitude_thresh' bigger than the negative peaks
                  mid = (p_p[sub_i][0] + n_p[i][0]) / 2
                  if binocular[mid] > binocular_thresh:  #to make sure the binocular eyes convergence is bigger than 30
                     striking_frame.append((mid, data[mid]))
                     break # just break the inner for loop and go back to the outer for loop
